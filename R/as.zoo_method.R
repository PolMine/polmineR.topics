...
#' @param regex a regex to extract the date from the document name
#' @param select topics to select
#' @param aggregation either "year", "quarter" or month
...
setMethod("as.zoo", "LDA", function(
  ...
  if (length(select) <= 1){
    ...
    DTcomb[, dummy := 1]
    DTcount <- DTcomb[, sum(dummy), by=.(date, topic1, topic2)]
